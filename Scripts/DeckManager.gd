...
# - サイクル開始時に手札を引き直し、マナが回復する
# - プレイヤーは手札からカードを戦場に配置（予約）する
# - サイクル終了時に予約された全ユニットが一斉に実体化する
extends Node
...
signal cycle_ended()                                      # サイクルが終了し、一斉スポーンする時
...
func _on_cycle_end() -> void:
	print("[DeckManager] ⏳ サイクル終了！ 予約ユニットが一斉に実体化します！")
	# BattleFieldに一斉スポーンを通知
	cycle_ended.emit()
